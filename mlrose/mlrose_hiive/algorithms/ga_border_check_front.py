...
# Author: Qingchuan Lyu (based code of genetic algorithm was written by Andrew Rollings and Genevieve Hayes in mlrose_hive)
...
# added by Lyu
def border_swap(next_gen, child):
    if len(next_gen) == 0:
        ...
        if new_start != last_start:
            ...
            child = np.logical_not(child).astype(int)
            next_gen.append(child)
    return next_gen
# edits end
...
@short_name('ga_border_front')
def genetic_border_front_alg(problem, pop_size=200, pop_breed_percent=0.75, elite_dreg_ratio=0.99,
    ...
    while (attempts < max_attempts) and (iters < max_iters):
        ...
        for _ in range(breeding_pop_size):
            ...
            # added by Lyu
            next_gen = border_swap(next_gen, child)
            # edits end
        ...
        if survivors_size > 0:
            ...
            for p in best_parents:
                curr_child = p[0]
                next_gen = border_swap(next_gen, curr_child)
            # edits end
